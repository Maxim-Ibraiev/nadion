...
import { IProduct } from '@/helpers/ProductStructure'
import useSelectedProducts from '@/hooks/useSelectedProducts'
import language from '@/language'
import { wrapper } from '@/redux/store'
import routes from '@/routes'
import { Box, MenuItem, Tab, Tabs, TabsOwnProps, Typography } from '@mui/material'
...
export default function Checkout() {
	...
	const formik = useFormik({
		...
		onSubmit: (e) => {
			if (postIndex === NOVA_POST_INDEX) formik.setFieldValue('ukrPostNumber', '')
			if (postIndex === UKR_POST_INDEX) formik.setFieldValue('novaPostNumber', '')

			console.log(e)
		},
	})

	const handleChange: TabsOwnProps['onChange'] = (e, v) => {
		setDelivaryIndex(v)
	}
	...
	return (
		<Layout>
			<div className={s.wrapper}>
				...
				<Box sx={{ width: 1, mx: 2, my: 4, borderBottom: 1, borderColor: 'divider' }}>
					<Tabs value={delivaryIndex} onChange={handleChange} className={s.line} aria-label="basic tabs example">
						<Tab sx={{ mx: 'auto' }} label={language.delivary} />
						<Tab sx={{ mx: 'auto' }} label={language.pickup} />
					</Tabs>

					{delivaryIndex === DELIVARY_POST_INDEX && (
						<Box sx={{ my: 4 }}>
							<Tabs sx={{ my: 4 }} value={postIndex} onChange={(e, v) => setPostIndex(v)} aria-label="basic tabs example">
					...
					{delivaryIndex === DELIVARY_PICK_UP_INDEX && (
						<Box sx={{ my: 4, display: 'grid', gridTemplateColumns: '1fr 250px' }}>
					...
					{delivaryIndex === DELIVARY_POST_INDEX && (
						<Box
							...
							style={{ display: 'grid', gridTemplateColumns: '20% 1fr', gap: '20px', marginBottom: '30px' }}
						>
							<Typography alignContent="center">{language.city}</Typography>
							<Form.Select formik={formik} name="city">
							...
							{postIndex === NOVA_POST_INDEX && (
								<Form.Select formik={formik} name="novaPostNumber">
									{['#1', '#10'].map((el) => (
										<MenuItem key={el} value={el}>
											{el}
										</MenuItem>
							...
							{postIndex === UKR_POST_INDEX && (
								<Form.Select formik={formik} name="ukrPostNumber">
							...
							<Form.Input formik={formik} name="name" multiline maxRows={4} />

							<Typography alignContent="center">{language.phoneNumber}</Typography>
							<Form.Input formik={formik} name="phoneNumber" multiline maxRows={4} />
